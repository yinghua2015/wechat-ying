...
export default {
  ...
  data() {
    return {
      ...
      leftPart: [{ name: "每日SHOW数量变化" }, { name: "每日SHOW充值数量" },{ name: "每日SHOW提现数量" }],
  ...
  mounted() {
    ...
    this.$http
      .get(`//trendapi.show-fund.com/show_ex_net_amount/SHOW/0/${timestamp}/1/`)
      .then(function(res) {
        console.log(res.data.show_day_net_amount);
        let btcRate = res.data.show_day_net_amount;
        let btcRate_amount = [];
        let btcRate_rate = [];
        for (let i = 0; i < btcRate.length; i++) {
          btcRate_amount.push([btcRate[i][0], Number(btcRate[i][1])]);
          btcRate_rate.push([btcRate[i][0], Number(btcRate[i][1])]);
        }
  ...
  methods: {
    ...
    allData(kv, index) {
      ...
      if (index == "2") {
        ...
        let timestamp = new Date().getTime();
        this.$http
          .get(`//trendapi.show-fund.com/show_ex_withdraw_amount/SHOW/0/${timestamp}/1/`)
          .then(function(res) {
            let btcRate = res.data.show_day_withdraw_amount;
            let btcRate_amount = [];
            let btcRate_rate = [];
            for (let i = 0; i < btcRate.length; i++) {
              btcRate_rate.push([btcRate[i][0], Number(btcRate[i][1])]);
        ...
        this.$http
          ...
          .then(function(res) {
            ...
            let usdRate = res.data.show_day_net_amount;
            let usdRate_amount = [];
            let usdRate_rate = [];
            for (let i = 0; i < usdRate.length; i++) {
              usdRate_rate.push([usdRate[i][0], Number(usdRate[i][1])]);
            }
            var names = ["借币总额", "借币利息"];
            var seriesOptions = [
              {
        ...
        let timestamp = new Date().getTime();
        this.$http
          .get(`//trendapi.show-fund.com/show_ex_deposit_amount/SHOW/0/${timestamp}/1/`)
          .then(function(res) {
            //console.log(res.data.bfx_lending_data);
            let usdRate = res.data.show_day_deposit_amount;
            let usdRate_amount = [];
            let usdRate_rate = [];
            for (let i = 0; i < usdRate.length; i++) {
              usdRate_rate.push([usdRate[i][0], Number(usdRate[i][1])]);
