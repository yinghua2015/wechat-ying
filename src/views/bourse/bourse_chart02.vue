<template>
  <div id="app">
    ...
    <div class="p-txt">{{txt_name}}流出{{this.$store.getters.ex}}交易所以太坊的数额。</div>
    <div v-show="isList2">
      <el-collapse>
        <el-collapse-item
...
export default {
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/eth_ex_withdraw_amount/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.eth_day_withdraw_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.eth_day_withdraw_amount[i][0],
              parseInt(Number(data.eth_day_withdraw_amount[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}流出${_this.$store.getters.ex}交易所ETH的数额`
            },
            annotations: [
              {
                labelOptions: {
