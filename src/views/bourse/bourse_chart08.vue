<template>
  <div id="app">
    ...
    <div class="p-txt">{{txt_name}}流出{{this.$store.getters.ex}}交易所的比特币的数额。</div>
    <div v-show="isList2">
      <el-collapse>
        <el-collapse-item
...
export default {
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/get_ex_day_withdraw/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.btc_day_withdraw.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.btc_day_withdraw[i][0],
              parseInt(Number(data.btc_day_withdraw[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}流出${_this.$store.getters.ex}交易所BTC数额`
            },
            annotations: [
              {
                labelOptions: {
