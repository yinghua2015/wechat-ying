<template>
  <div id="app">
   ...
   <div class="p-txt">{{txt_name}}流入{{this.$store.getters.ex}}交易所USDT的数额。</div>
<div v-show="isList2">
        <el-collapse>
...
export default {
  ...
  watch: {
    ...
    ex: function(val) {
      ...
      if (val === "Okex") {
        var localcontent = "usdt-in-ok";
        this.typeNameNew = "usdt-in-ok";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "Huobi") {
        var localcontent = "usdt-in-hb";
        this.typeNameNew = "usdt-in-hb";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "Bitfinex") {
        var localcontent = "usdt-in-btf";
        this.typeNameNew = "usdt-in-btf";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "Binance") {
        var localcontent = "usdt-in-bin";
        this.typeNameNew = "usdt-in-bin";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "all") {
        var localcontent = "usdt-in-all";
        this.typeNameNew = "usdt-in-all";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      }
    }
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var endTime = Date.parse(new Date());
      var urt_dev = "/goods/usdt-in";
      var urt_pro =
        "http://trendapi.show-fund.com/usdt_ex_deposit_amount/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.usdt_day_deposit_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.usdt_day_deposit_amount[i][0],
              parseInt(Number(data.usdt_day_deposit_amount[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}流入${_this.$store.getters.ex}交易所USDT数额`
            },
            annotations: [
              {
                labelOptions: {
