<template>
  <div id="app">
    ...
    <div class="p-txt">{{txt_name}}交易所SHOW净流入数额=流入交易所SHOW的数额-流出交易所SHOW的数额。统计了包括okex、digifinex、hcoin、fcoin、cex、bigone等交易所。</div>
<div v-show="isList2">
        <el-collapse>
    ...
    <div class="form-page">
        ...
        <div class="wrapper bg-white b">
            <editer ref="editer" v-on:reload="exchange"  :isday="isday" :markTime="value1" :isList="isList" :timesBengin="times_begin"  :yAxi="yAxi" :typeNameNew="typeNameNew"></editer>
        </div>
    </div>
  </div>
</template>
...
export default {
    ...
    ex: function(val) {
      ...
      if (this.type === "柱形图") {
        ...
        if (val === "Okex" && type_change === "column") {
          var localcontent = "show-net-ok-z";
          this.typeNameNew = "show-net-ok-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Huobi" && type_change === "column") {
          var localcontent = "show-net-hb-z";
          this.typeNameNew = "show-net-hb-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Bitfinex" && type_change === "column") {
          var localcontent = "show-net-btf-z";
          this.typeNameNew = "show-net-btf-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Binance" && type_change === "column") {
          var localcontent = "show-net-bin-z";
          this.typeNameNew = "show-net-bin-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "all" && type_change === "column") {
          var localcontent = "show-net-all-z";
          this.typeNameNew = "show-net-all-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        }
      } else {
        ...
        if (val === "Okex" && type_change === "") {
          var localcontent = "show-net-ok-l";
          this.typeNameNew = "show-net-ok-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Huobi" && type_change === "") {
          var localcontent = "show-net-hb-l";
          this.typeNameNew = "show-net-hb-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Bitfinex" && type_change === "") {
          var localcontent = "show-net-btf-l";
          this.typeNameNew = "show-net-btf-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Binance" && type_change === "") {
          var localcontent = "show-net-bin-l";
          this.typeNameNew = "show-net-bin-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "all" && type_change === "") {
          var localcontent = "show-net-all-l";
          this.typeNameNew = "show-net-all-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        }
      }
  ...
  methods: {
    ...
    createChart(flag, flag2, type_change) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/show_ex_net_amount/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            series_all = [],
            volume = [],
            dataLength = data.show_day_net_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.show_day_net_amount[i][0],
              parseInt(Number(data.show_day_net_amount[i][1])) // the date
            ]);
          }
          // create the chart
          series_all.push({
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}交易所SHOW净流入数额`
            },
            annotations: [
              {
                labelOptions: {
    ...
    moreChart() {
      ...
      if (this.type === "柱形图") {
        var type_change = "column";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'show-net-ok-z');
        this.exchange(type_change);
      } else{
        if(this.$store.getters.ex === "Okex") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'show-net-ok-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "Binance") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'show-net-bin-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "Bitfinex") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'show-net-btf-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "Huobi") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'show-net-hb-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "all") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'show-net-all-l');
        this.exchange(type_change);
      }
      }
    },
