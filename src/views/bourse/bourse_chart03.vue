<template>
  <div id="app">
    ...
    <div class="p-txt">{{txt_name}}流入交易所秀币的数额。统计了包括okex、digifinex、hcoin、fcoin、cex、bigone等交易所。</div>
<div v-show="isList2">
        <el-collapse>
...
export default {
  ...
  watch: {
    ...
    ex: function(val) {
      ...
      if (val === "Okex") {
        var localcontent = "show-in-ok";
        this.typeNameNew = "show-in-ok";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "Huobi") {
        var localcontent = "show-in-hb";
        this.typeNameNew = "show-in-hb";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "Bitfinex") {
        var localcontent = "show-in-btf";
        this.typeNameNew = "show-in-btf";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "Binance") {
        var localcontent = "show-in-bin";
        this.typeNameNew = "show-in-bin";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "all") {
        var localcontent = "show-in-all";
        this.typeNameNew = "show-in-all";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      }
    }
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/show_ex_deposit_amount/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.show_day_deposit_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.show_day_deposit_amount[i][0],
              parseInt(Number(data.show_day_deposit_amount[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}流入交易所SHOW数额`
            },
            annotations: [
              {
                labelOptions: {
