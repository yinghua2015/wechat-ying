  ...
  <div id="app">
    <div class>
      ...
      <select @change="moreChart()" v-model="type">
        <option v-for="s in moreSamples" :value="s.name">{{s.name}}</option>
      </select>
    ...
    <div class="p-txt">ETH净流入数额=流入交易所ETH的数额-流出交易所ETH的数额。</div>
    <div v-show="isList2">
      <el-collapse>
        <el-collapse-item
          :title="tagTxt+item.label_content"
          :name="index"
        ...
        >
          ...
          <div class="t-btn">
            ...
            <updata2
              v-show="isshow2"
              ref="updata2"
              v-on:reload="exchange"
              :isday="isday"
              :typeNameNew="typeNameNew"
              :markTime="value1"
              :contentlabel="content2"
              :recordlabel="record_id2"
              :isList="isList"
              :timesBengin="times_begin"
              :yAxi="yAxi"
            ></updata2>
          </div>
        </el-collapse-item>
    ...
    <div v-show="isList">
      ...
      <el-collapse v-for="(item, index) in markList">
        <el-collapse-item
          :title="index+1+logsTag+item.topic_content"
          :name="index"
        ...
        >
          ...
          <div class="t-btn">
            ...
            <el-button
              type="button"
              @click="updata_new(item.record_id,item.mark_content,item.topic_content)"
            >更新</el-button>
        ...
        <el-collapse-item
          ...
          style="background:#0080ff !important"
        >
          <div v-html="item.mark_content"></div>
          <div class="t-btn">
            <div>编辑于：{{item.update_time}}</div>
...
export default {
  ...
  watch: {
    times: function(val) {
      ...
      console.log(val);
      if (this.$store.getters.times == 0) {
        this.txt_name = "每小时";
      } else if (this.$store.getters.times == 1) {
        this.txt_name = "每日";
      }
      this.isday = val;
      if (this.type === "柱形图") {
        var type_change = "column";
        this.$store.commit("typeStatus", type_change);
        this.exchange(type_change);
      } else {
        var type_change = "";
        this.$store.commit("typeStatus", type_change);
        this.exchange(type_change);
    ...
    ex: function(val) {
      ...
      if (this.type === "柱形图") {
        ...
        this.$store.commit("typeStatus", type_change);
        //this.exchange(type_change)
        if (val === "Okex" && type_change === "column") {
          var localcontent = "eth-net-ok-z";
          this.typeNameNew = "eth-net-ok-z";
          this.$store.commit("typeNewAll", this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Huobi" && type_change === "column") {
          var localcontent = "eth-net-hb-z";
          this.typeNameNew = "eth-net-hb-z";
          this.$store.commit("typeNewAll", this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Bitfinex" && type_change === "column") {
          var localcontent = "eth-net-btf-z";
          this.typeNameNew = "eth-net-btf-z";
          this.$store.commit("typeNewAll", this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Binance" && type_change === "column") {
          var localcontent = "eth-net-bin-z";
          this.typeNameNew = "eth-net-bin-z";
          this.$store.commit("typeNewAll", this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "all" && type_change === "column") {
          var localcontent = "eth-net-all-z";
          this.typeNameNew = "eth-net-all-z";
          this.$store.commit("typeNewAll", this.typeNameNew);
          this.exchange(type_change);
        }
      } else {
        var type_change = "";
        this.$store.commit("typeStatus", type_change);
        if (val === "Okex" && type_change === "") {
          var localcontent = "eth-net-ok-l";
          this.typeNameNew = "eth-net-ok-l";
          this.$store.commit("typeNewAll", this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Huobi" && type_change === "") {
          var localcontent = "eth-net-hb-l";
          this.typeNameNew = "eth-net-hb-l";
          this.$store.commit("typeNewAll", this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Bitfinex" && type_change === "") {
          var localcontent = "eth-net-btf-l";
          this.typeNameNew = "eth-net-btf-l";
          this.$store.commit("typeNewAll", this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Binance" && type_change === "") {
          var localcontent = "eth-net-bin-l";
          this.typeNameNew = "eth-net-bin-l";
          this.$store.commit("typeNewAll", this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "all" && type_change === "") {
          var localcontent = "eth-net-all-l";
          this.typeNameNew = "eth-net-all-l";
          this.$store.commit("typeNewAll", this.typeNameNew);
          this.exchange(type_change);
        }
      }
...
if (this.$store.getters.times == 0) {
    ...
    if (this.type === "柱形图") {
      var type_change = "column";
      if (this.$store.getters.ex === "Okex") {
        this.$store.commit("typeStatus", type_change);
        var localcontent = "eth-net-ok-z";
        this.typeNameNew = "eth-net-ok-z";
        this.$store.commit("typeNewAll", this.typeNameNew);
        this.exchange(type_change);
      } else if (this.$store.getters.ex === "Huobi") {
        this.$store.commit("typeStatus", type_change);
        var localcontent = "eth-net-hb-z";
        this.typeNameNew = "eth-net-hb-z";
        this.$store.commit("typeNewAll", this.typeNameNew);
        this.exchange(type_change);
      } else if (this.$store.getters.ex === "Bitfinex") {
        this.$store.commit("typeStatus", type_change);
        var localcontent = "eth-net-btf-z";
        this.typeNameNew = "eth-net-btf-z";
        this.$store.commit("typeNewAll", this.typeNameNew);
        this.exchange(type_change);
      } else if (this.$store.getters.ex === "Binance") {
        this.$store.commit("typeStatus", type_change);
        var localcontent = "eth-net-bin-z";
        this.typeNameNew = "eth-net-bin-z";
        this.$store.commit("typeNewAll", this.typeNameNew);
        this.exchange(type_change);
      } else if (this.$store.getters.ex === "all") {
        this.$store.commit("typeStatus", type_change);
        var localcontent = "eth-net-all-z";
        this.typeNameNew = "eth-net-all-z";
        this.$store.commit("typeNewAll", this.typeNameNew);
  ...
  methods: {
    ...
    exchange(type_change) {
      ...
      this.$http
        ...
        .then(function(response) {
          ...
          console.log(flag);
          _that.get_label(flag, type_change);
        });
    },
    first(type_change) {
      const _that = this;
        ...
        .then(function(response) {
          ...
          for (i; i < res.length; i += 1) {
            if (_that.isday === "1") {
              ...
              _that.flagsName.push(res[i].mark_time);
              //console.log(_that.flagsName + "按天的时间");
            } else if (_that.isday === "1") {
              flag.push({
                x: res[i].mark_time,
                title: "1"
              });
              //console.log(_that.flagsName + "按小时的时间");
            }
          }
          _that.createChart(flag, type_change);
        });
    },
    timeer(time_e) {
    ...
    createChart(flag, flag2, type_change) {
      const _this = this;
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/eth_ex_net_amount/" +
        this.$store.getters.ex +
        "/" +
        "0/" +
        endTime +
        "/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            series_all = [],
            volume = [],
            dataLength = data.eth_day_net_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.eth_day_net_amount[i][0],
              parseInt(Number(data.eth_day_net_amount[i][1])) // the date
            ]);
          }
          // create the chart
          series_all.push({
            type: _this.$store.getters.typeS,
            name: "eth提现",
            data: ohlc,
            id: "dataseries",
            color: "green",
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}${_this.$store.getters.ex}交易所ETH净流入数量`
            },
            annotations: [
              {
                labelOptions: {
            ...
            tooltip: {
              ...
              formatter: function() {
                ...
                var m = date.getMinutes();
                var s = date.getSeconds();
                if(_this.$store.getters.times == 0){
                  var content = Y + M + D + h;
                }else if(_this.$store.getters.times == 1){
                  var content = Y + M + D;
                }
    ...
    moreChart() {
      ...
      if (this.type === "柱形图") {
        var type_change = "column";
        this.$store.commit("typeStatus", type_change);
        this.$store.commit("typeNewAll", "eth-net-ok-z");
        this.exchange(type_change);
      } else {
        if (this.$store.getters.ex === "Okex") {
          var type_change = "";
          this.$store.commit("typeStatus", type_change);
          this.$store.commit("typeNewAll", "eth-net-ok-l");
          this.exchange(type_change);
        } else if (this.$store.getters.ex === "Binance") {
          var type_change = "";
          this.$store.commit("typeStatus", type_change);
          this.$store.commit("typeNewAll", "eth-net-bin-l");
          this.exchange(type_change);
        } else if (this.$store.getters.ex === "Bitfinex") {
          var type_change = "";
          this.$store.commit("typeStatus", type_change);
          this.$store.commit("typeNewAll", "eth-net-btf-l");
          this.exchange(type_change);
        } else if (this.$store.getters.ex === "Huobi") {
          var type_change = "";
          this.$store.commit("typeStatus", type_change);
          this.$store.commit("typeNewAll", "eth-net-hb-l");
          this.exchange(type_change);
        } else if (this.$store.getters.ex === "all") {
          var type_change = "";
          this.$store.commit("typeStatus", type_change);
          this.$store.commit("typeNewAll", "eth-net-all-l");
          this.exchange(type_change);
        }
      }
    },
