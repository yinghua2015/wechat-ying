<template>
  <div id="app">
    ...
    <div class="p-txt">{{txt_name}}流入{{this.$store.getters.ex}}交易所比特币的数额</div>
    <div v-show="isList2">
      <el-collapse>
        <el-collapse-item
...
export default {
  ...
  watch: {
    ...
    ex: function(val) {
      ...
      if (val === "Okex") {
        var localcontent = "btc-in-ok";
        this.typeNameNew = "btc-in-ok";
        this.$store.commit("typeNewAll", this.typeNameNew);
        this.exchange();
      } else if (val === "Huobi") {
        var localcontent = "btc-in-hb";
        this.typeNameNew = "btc-in-hb";
        this.$store.commit("typeNewAll", this.typeNameNew);
        this.exchange();
      } else if (val === "Bitfinex") {
        var localcontent = "btc-in-btf";
        this.typeNameNew = "btc-in-btf";
        this.$store.commit("typeNewAll", this.typeNameNew);
        this.exchange();
      } else if (val === "Binance") {
        var localcontent = "btc-in-bin";
        this.typeNameNew = "btc-in-bin";
        this.$store.commit("typeNewAll", this.typeNameNew);
        this.exchange();
      } else if (val === "all") {
        var localcontent = "btc-in-all";
        this.typeNameNew = "btc-in-all";
        this.$store.commit("typeNewAll", this.typeNameNew);
        this.exchange();
      }
    }
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/get_ex_day_deposit/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.btc_day_deposit.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.btc_day_deposit[i][0],
              parseInt(Number(data.btc_day_deposit[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
            name: "流入交易所" + _this.$store.getters.ex + "数量",
            data: ohlc,
            id: "dataseries",
            color: "green",
            lineColor: "green",
  ...
  created() {
    ...
    //   this.createChart();
    // }
    console.log(this.$store.getters.ex);
    if (window.innerWidth < 500) {
