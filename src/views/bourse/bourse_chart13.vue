<template>
  <div id="app">
    ...
    <div class="">
     ...
     <div class="p-txt">USDT净流入数额=流入交易所USDT的数额-流出交易所USDT的数额。</div>
<div v-show="isList2">
        <el-collapse>
...
export default {
    ...
    ex: function(val) {
      ...
      if (this.type === "柱形图") {
        ...
        if (val === "Okex" && type_change === "column") {
          var localcontent = "usdt-net-ok-z";
          this.typeNameNew = "usdt-net-ok-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Huobi" && type_change === "column") {
          var localcontent = "usdt-net-hb-z";
          this.typeNameNew = "usdt-net-hb-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Bitfinex" && type_change === "column") {
          var localcontent = "usdt-net-btf-z";
          this.typeNameNew = "usdt-net-btf-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Binance" && type_change === "column") {
          var localcontent = "usdt-net-bin-z";
          this.typeNameNew = "usdt-net-bin-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "all" && type_change === "column") {
          var localcontent = "usdt-net-all-z";
          this.typeNameNew = "usdt-net-all-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        }
      } else {
        ...
        if (val === "Okex" && type_change === "") {
          var localcontent = "usdt-net-ok-l";
          this.typeNameNew = "usdt-net-ok-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Huobi" && type_change === "") {
          var localcontent = "usdt-net-hb-l";
          this.typeNameNew = "usdt-net-hb-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Bitfinex" && type_change === "") {
          var localcontent = "usdt-net-btf-l";
          this.typeNameNew = "usdt-net-btf-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Binance" && type_change === "") {
          var localcontent = "usdt-net-bin-l";
          this.typeNameNew = "usdt-net-bin-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "all" && type_change === "") {
          var localcontent = "usdt-net-all-l";
          this.typeNameNew = "usdt-net-all-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        }
      }
  ...
  methods: {
    ...
    createChart(flag, flag2, type_change) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/usdt_ex_net_amount/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            series_all = [],
            volume = [],
            dataLength = data.usdt_day_net_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.usdt_day_net_amount[i][0],
              parseInt(Number(data.usdt_day_net_amount[i][1])) // the date
            ]);
          }
          // create the chart
          series_all.push({
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}${_this.$store.getters.ex}交易所USDT净流入的数额`
            },
            annotations: [
              {
                labelOptions: {
    ...
    moreChart() {
      ...
      if (this.type === "柱形图") {
        var type_change = "column";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'usdt-net-ok-z');
        this.exchange(type_change);
      } else{
        if(this.$store.getters.ex === "Okex") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'usdt-net-ok-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "Binance") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'usdt-net-bin-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "Bitfinex") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'usdt-net-btf-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "Huobi") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'usdt-net-hb-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "all") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'usdt-net-all-l');
        this.exchange(type_change);
      }
      }
    },
