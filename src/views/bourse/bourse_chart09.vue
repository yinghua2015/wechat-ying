<template>
  <div id="app">
    ...
    <div class="p-txt">BTC净流入数额=流入交易所BTC的数额-流出交易所BTC的数额。</div>
<div v-show="isList2">
        <el-collapse>
    ...
    <div class="form-page">
        ...
        <div class="wrapper bg-white b">
            <editer ref="editer" v-on:reload="exchange"  :isday="isday" :markTime="value1" :isList="isList" :timesBengin="times_begin" :typeNameNew="typeNameNew" :yAxi="yAxi"></editer>
        </div>
    </div>
  </div>
</template>
...
export default {
    ...
    ex: function(val) {
      ...
      if (this.type === "柱形图") {
        ...
        if (val === "Okex" && type_change === "column") {
          var localcontent = "btc-net-ok-z";
          this.typeNameNew = "btc-net-ok-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Huobi" && type_change === "column") {
          var localcontent = "btc-net-hb-z";
          this.typeNameNew = "btc-net-hb-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Bitfinex" && type_change === "column") {
          var localcontent = "btc-net-btf-z";
          this.typeNameNew = "btc-net-btf-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Binance" && type_change === "column") {
          var localcontent = "btc-net-bin-z";
          this.typeNameNew = "btc-net-bin-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "all" && type_change === "column") {
          var localcontent = "btc-net-all-z";
          this.typeNameNew = "btc-net-all-z";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        }
      } else {
        ...
        if (val === "Okex" && type_change === "") {
          var localcontent = "btc-net-ok-l";
          this.typeNameNew = "btc-net-ok-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Huobi" && type_change === "") {
          var localcontent = "btc-net-hb-l";
          this.typeNameNew = "btc-net-hb-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Bitfinex" && type_change === "") {
          var localcontent = "btc-net-btf-l";
          this.typeNameNew = "btc-net-btf-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "Binance" && type_change === "") {
          var localcontent = "btc-net-bin-l";
          this.typeNameNew = "btc-net-bin-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        } else if (val === "all" && type_change === "") {
          var localcontent = "btc-net-all-l";
          this.typeNameNew = "btc-net-all-l";
          this.$store.commit("typeNewAll",this.typeNameNew);
          this.exchange(type_change);
        }
      }
  ...
  methods: {
    ...
    first(type_change) {
      ...
      this.$http
        ...
        .then(function(response) {
          ...
          // 	}
          for (i; i < res.length; i += 1) {
            if (_that.$store.getters.times == 1) {
              flag.push({
                x: res[i].mark_time,
                title: res[i].count
              });
            ...
            } else if (_that.$store.getters.times == 1) {
              flag.push({
                x: res[i].mark_time,
                title: "1"
              });
    ...
    createChart(flag, flag2, type_change) {
      ...
      var endTime = Date.parse(new Date());
      var urt_dev = "/goods/btc-in";
      var urt_pro =
        "http://trendapi.show-fund.com/get_ex_day_net/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            series_all = [],
            volume = [],
            dataLength = data.btc_day_net.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.btc_day_net[i][0],
              parseInt(Number(data.btc_day_net[i][1])) // the date
            ]);
          }
          // create the chart
          series_all.push({
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}${_this.$store.getters.ex}交易所BTC净流入数额`
            },
            annotations: [
              {
                labelOptions: {
    ...
    moreChart() {
      ...
      if (this.type === "柱形图") {
        var type_change = "column";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'btc-net-ok-z');
        this.exchange(type_change);
      } else{
        if(this.$store.getters.ex === "Okex") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'btc-net-ok-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "Binance") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'btc-net-bin-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "Bitfinex") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'btc-net-btf-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "Huobi") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'btc-net-hb-l');
        this.exchange(type_change);
      }else if(this.$store.getters.ex === "all") {
        var type_change = "";
        this.$store.commit("typeStatus",type_change);
        this.$store.commit("typeNewAll",'btc-net-all-l');
        this.exchange(type_change);
      }
      }
    },
