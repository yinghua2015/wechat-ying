<template>
  <div id="app">
    ...
    <div class="p-txt">{{txt_name}}流入{{this.$store.getters.ex}}交易所以太坊的数额。</div>
    <div v-show="isList2">
      <el-collapse>
        <el-collapse-item
...
export default {
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/eth_ex_deposit_amount/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.eth_day_deposit_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.eth_day_deposit_amount[i][0],
              parseInt(Number(data.eth_day_deposit_amount[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}流入${_this.$store.getters.ex}交易所ETH的数额`
            },
            annotations: [
              {
                labelOptions: {
  ...
  created() {
    if (this.$store.getters.ex === "Okex") {
      this.$store.getters.typeNew === "eth-in-ok";
      this.typeNew = this.$store.getters.typeNew;
    } else if (this.$store.getters.ex === "Huobi") {
      this.$store.getters.typeNew === "eth-in-hb";
      this.typeNew = this.$store.getters.typeNew;
    }
