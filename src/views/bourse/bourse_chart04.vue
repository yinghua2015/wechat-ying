<template>
  <div id="app">
    ...
    <div class="p-txt">{{txt_name}}流出交易所秀币的数额。统计了包括okex、digifinex、hcoin、fcoin、cex、bigone等交易所。</div>
<div v-show="isList2">
        <el-collapse>
...
export default {
  ...
  watch: {
    ...
    ex: function(val) {
      ...
      if (val === "Okex") {
        var localcontent = "show-out-ok";
        this.typeNameNew = "show-out-ok";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "Huobi") {
        var localcontent = "show-out-hb";
        this.typeNameNew = "show-out-hb";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "Bitfinex") {
        var localcontent = "show-out-btf";
        this.typeNameNew = "show-out-btf";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "Binance") {
        var localcontent = "show-out-bin";
        this.typeNameNew = "show-out-bin";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      } else if (val === "all") {
        var localcontent = "show-out-all";
        this.typeNameNew = "show-out-all";
        this.$store.commit("typeNewAll",this.typeNameNew);
        this.exchange();
      }
    }
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var endTime = Date.parse(new Date());
      var urt_dev = "/goods/show-out";
      var urt_pro =
        "http://trendapi.show-fund.com/show_ex_withdraw_amount/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.show_day_withdraw_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.show_day_withdraw_amount[i][0],
              parseInt(Number(data.show_day_withdraw_amount[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}流出交易所SHOW数额`
            },
            annotations: [
              {
                labelOptions: {
