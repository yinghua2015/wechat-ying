<template>
  <div id="app">
    ...
    <div class="p-txt">数据来源：//coinmarketcap.com/currencies/show/</div>
    <div v-show="isList2">
      <el-collapse>
        <el-collapse-item
...
export default {
  ...
  watch: {
    ...
    //   if (val === "Okex") {
    //     var localcontent = "eth-out-ok";
    //     this.typeNameNew = "eth-out-ok";
    //     this.$store.commit("typeNewAll",this.typeNameNew);
    //     this.exchange();
    //   } else if (val === "Huobi") {
    //     var localcontent = "eth-out-hb";
    //     this.typeNameNew = "eth-out-hb";
    //     this.$store.commit("typeNewAll",this.typeNameNew);
    //     this.exchange();
    //   } else if (val === "Bitfinex") {
    //     var localcontent = "eth-out-btf";
    //     this.typeNameNew = "eth-out-btf";
    //     this.$store.commit("typeNewAll",this.typeNameNew);
    //     this.exchange();
    //   } else if (val === "Binance") {
    //     var localcontent = "eth-out-bin";
    //     this.typeNameNew = "eth-out-bin";
    //     this.$store.commit("typeNewAll",this.typeNameNew);
    //     this.exchange();
    //   } else if (val === "all") {
    //     var localcontent = "eth-out-all";
    //     this.typeNameNew = "eth-out-all";
    //     this.$store.commit("typeNewAll",this.typeNameNew);
    //     this.exchange();
    //   }
    // }
  ...
  mounted() {
    ...
    this.typeNameNew = "okex-show-eth";
    this.$store.commit("typeNewAll", this.typeNameNew);
    this.exchange();
  },
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var endTime = Date.parse(new Date());
      var urt_dev = "/goods/eth-out";
      // var urt_pro =
      //   "http://trendapi.show-fund.com/coinmarketpricebytype/show/eth/0/" + endTime
      //   // "/" +
      //   // this.$store.getters.times;
      var urt_pro = "";
      // "http://trendapi.show-fund.com/coinmarketpricebytype/show/btc/hour/0/" +
      // endTime;
      if (this.$store.getters.times == 1) {
        urt_pro =
          "http://trendapi.show-fund.com/coinmarketpricebytype/show/eth/day/1515600000000/" +
          endTime;
        //return urt_pro;
      } else if (this.$store.getters.times == 0) {
        urt_pro =
          "http://trendapi.show-fund.com/coinmarketpricebytype/show/eth/hour/1515600000000/" +
          endTime;
        //return urt_pro;
      }
      console.log(urt_pro);
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data[i][0],
              Number(data[i][1] * 10000) // the date
            ]);
          }
          
          // let dyq = _this.show_eth_data.concat(ohlc)
          // console.log(dyq)
          // console.log([...ohlc,..._this.show_eth_data])
          series_all.push({
            type: "line",
            name: "okex-show-eth",
            data: ohlc,
          ...
          this.stockChart = new Highcharts.stockChart("container", {
            ...
            title: {
              text: `${_this.txt_name}秀币对以太坊价格走势图`
            },
            annotations: [
              {
                labelOptions: {
            ...
            tooltip: {
              ...
              formatter: function() {
                ...
                var s = date.getSeconds();
                var content = Y + M + D + h;
                var content_hour = Y + M + D;
                console.log(content);
                _this.times_begin = content;
                _this.get_time(timeNow, content, logsContent);
                var re = /(?=(?!(\b))(\d{3})+$)/g;
                var dy = String(this.y).replace(re, ","); //老版本代码
                if (_this.$store.getters.times == 1) {
                  return (
                  "<b>" +
                  content_hour +
                  "</b> :  <b>" +
                  Number(this.y).toFixed(5) +
                  "</b>" +
                  " eth"
                );
                } else if (_this.$store.getters.times == 0) {
                  return (
                  "<b>" +
                  content +
                  "</b> :  <b>" +
  ...
  //       .then(function(res) {
  //         console.log(res.data)
  //         _this.eth_data = res.data
  //           for(let i =0;i<_this.eth_data.length;i++){
  //       let ele = _this.show_data[i][1] / _this.eth_data[i][1]
  //       _this.show_eth_data.push([_this.eth_data[i][0],ele])
  //       }
  //       console.log(JSON.stringify(_this.show_eth_data))
