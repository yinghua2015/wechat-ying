<template>
  <div id="app">
    ...
    <div class="p-txt">以太坊区块链上，每日SHOW转账的总数额，单位是SHOW。A地址转给B地址X个SHOW，即转账SHOW数额为X个SHOW。每日所有转账数额之和即每日SHOW转账的总数额。</div>
<div v-show="isList2">
        <el-collapse>
...
export default {
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/show_tx_amount" +
        "/" +
        "0/" +
        endTime +
        "/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.show_tx_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.show_tx_amount[i][0],
              parseInt(Number(data.show_tx_amount[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
