<template>
  <div id="app">
    ...
    <div class="p-txt">
      比特币区块链上，每日比特币转账交易的总笔数。A地址转给B地址一笔任意数量比特币，即为一笔转账。每日所有转账笔数之和即每日比特币转账交易的总笔数。
    </div>
<div v-show="isList2">
        <el-collapse>
...
export default {
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/get_btc_day_count" +
        "/" +
        "0/" +
        endTime +
        "/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.btc_day_tx_count.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.btc_day_tx_count[i][0],
              parseInt(Number(data.btc_day_tx_count[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
