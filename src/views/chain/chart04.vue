<template>
  <div id="app">
    ...
    <div class="p-txt">
以太坊区块链上，每日以太坊转账交易的总笔数。A地址转给B地址一笔任意数量以太坊，即为一笔转账。每日所有转账笔数之和即每日以太坊转账交易的总笔数。
 </div>
<div v-show="isList2">
        <el-collapse>
...
export default {
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/eth_tx_count" +
        "/" +
        "0/" +
        endTime +
        "/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.eth_day_tx_count.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.eth_day_tx_count[i][0],
              parseInt(Number(data.eth_day_tx_count[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
