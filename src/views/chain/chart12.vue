<template>
  <div id="app">
    ...
    <div class="p-txt">首次发生ETH交易的地址数（不包含从未发生过ETH交易的地址）。</div>
<div v-show="isList2">
        <el-collapse>
...
export default {
  ...
  mounted() {
    ...
    this.typeNameNew = "eth-new-address"
    this.$store.commit("typeNewAll",this.typeNameNew);
    this.exchange();
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro = "http://trendapi.show-fund.com/eth_day_birth_address/";
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.data[i][0],
              parseInt(Number(data.data[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
