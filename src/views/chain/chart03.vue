<template>
  <div id="app">
    ...
    <div class="p-txt">
      以太坊区块链上，每日以太坊转账的总数额，单位是ETH。A地址转给B地址X个以太坊，即转账以太坊数额为X个ETH。每日所有转账数额之和即每日以太坊转账的总数额。
    </div>
<div v-show="isList2">
        <el-collapse>
...
export default {
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/eth_tx_amount" +
        "/" +
        "0/" +
        endTime +
        "/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.eth_day_tx_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.eth_day_tx_amount[i][0],
              parseInt(Number(data.eth_day_tx_amount[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
