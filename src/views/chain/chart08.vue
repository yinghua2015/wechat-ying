<template>
  <div id="app">
    ...
    <div class="p-txt">以太坊区块链上，每日SHOW转账交易的总笔数。A地址转给B地址一笔任意数量SHOW，即为一笔转账。每日所有转账笔数之和即每日SHOW转账交易的总笔数。</div>
<div v-show="isList2">
        <el-collapse>
...
export default {
  ...
  mounted() {
    this.typeNameNew = "show-deal-num"
    this.$store.commit("typeNewAll",this.typeNameNew);
    this.exchange();
  },
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/show_tx_count" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.show_tx_count.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.show_tx_count[i][0],
              parseInt(Number(data.show_tx_count[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
