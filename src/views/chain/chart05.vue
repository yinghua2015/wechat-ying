<template>
  <div id="app">
    ...
    <div class="p-txt">Omini区块链上，每日USDT转账的总数额，单位是USDT。A地址转给B地址X个USDT，即转账USDT数额为X个USDT。每日所有转账数额之和即每日USDT转账的总数额。</div>
    <div v-show="isList2">
        <el-collapse>
...
export default {
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro =
        "http://trendapi.show-fund.com/usdt_tx_amount" +
        "/" +
        "0/" +
        endTime +
        "/" +
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var ohlc = [],
            ...
            volume = [],
            _flag = [],
            dataLength = data.usdt_day_tx_amount.length,
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.usdt_day_tx_amount[i][0],
              parseInt(Number(data.usdt_day_tx_amount[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "column",
