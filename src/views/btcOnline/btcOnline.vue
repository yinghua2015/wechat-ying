<template>
  <div id="app">
   ...
   <div>Coinmarketcap统计BTC的总市值。当前某币种总市值=当前流通总量*当前币价。数据来源：//coinmarketcap.com/zh/currencies/bitcoin/</div>
  </div>
</template>
<style scoped>
.form-page {
...
export default {
  ...
  methods: {
    ...
    createChart(flag, flag2) {
      ...
      var urt_pro = "http://trendapi.show-fund.com/get_btc_currencies/";
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          for (i; i < dataLength; i += 1) {
            ohlc.push([
              data.market_cap_by_available_supply[i][0],
              parseInt(Number(data.market_cap_by_available_supply[i][1])) // the date
            ]);
            volume.push([
              data.volume_usd[i][0],
              parseInt(Number(data.volume_usd[i][1])) // the date
            ]);
          }
          series_all.push({
            type: "line",
