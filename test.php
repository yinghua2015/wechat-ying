...
function parse_html($data)
{
	global $gloable_ether_dataBaseLink;
	$arrListNew = array();
	$address_exp = "/0x\w{40}\?a=0x\w{40}/";
	// /token\/0x\w{40}/
	preg_match_all($address_exp, $data, $matches_address);
	// echo "<pre>";print_r($matches_address);echo "<pre>";
	// 	return;
	$num = count($matches_address[0]);
	for ($i = 0; $i < $num; $i++) {
		$address = $matches_address[0][$i];
		$adr = str_replace("0xf41861f194e7ba8de95144a89e0c6ed16ee0b3a0?a=", "", $address);
		// echo "<pre>";print_r($adr);echo "<pre>";
		// return;
	...
	// echo "<pre>";print_r($arrListNew);echo "<pre>";
	// 	return;
	$amount_exp = "/\<td>(\d*(,\d{3})*(.(\d{3},)*\d+)?)<\/td>/";
...
function insert_new_data($link, $row_data)
{
	$linkether2 = mysqli_connect('rm-j6c0xl840e2b18386.mysql.rds.aliyuncs.com', 'ether', 'n9483E6wBUFt5z6', 'ether') or die('Connect Error:' . mysqli_connect_errno() . ":" . mysqli_connect_error());

	$sql = sprintf("INSERT INTO ether.showData (address,name,class,quantity,percentage,createtime,updatetime) VALUES('%s','%s','%s',%s,%s,'%s','%s');", $row_data[0], $row_data[1], $row_data[2], $row_data[3], $row_data[4], '2016-12-25 11:46:13', '2016-12-25 11:46:13');
	// echo $row_data[0];
	// return;
...
{
	// echo "<pre>";print_r($result_list);echo "<pre>";
	// 	return;
	global $link;
	$address_list = "";
